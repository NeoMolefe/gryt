    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          />
          <motion.div
            className="fixed bottom-0 left-0 right-0 z-50 max-h-[75vh] overflow-y-auto rounded-t-2xl border-t border-border bg-card p-6 pb-[calc(24px_+_env(safe-area-inset-bottom))]"
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
